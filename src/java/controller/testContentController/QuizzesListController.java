...
public class QuizzesListController extends AuthorizationController {
    ...
    protected void processGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("view/test_content/newQuizzesList.jsp").forward(request, response);
    }
    ...
    protected void processPost(HttpServletRequest request, HttpServletResponse response)
        ...
        log(quizName);
        String quizType = "";
        if (request.getParameterValues("QuizType") != null) {
            String[] arr = request.getParameterValues("QuizType");
            if (arr.length != 0) {
                for (String string : arr) {
                    quizType += string + ",";
                }
            }
            quizType = quizType.substring(0, quizType.trim().length() - 1);
        }
